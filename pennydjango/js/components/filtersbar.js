...
export class FiltersBar extends React.Component {
    ...
    renderFilters(filters) {
        ...
        return [
            ...
            <div key="searchingText" className='filter-container'>{searchingText(searching_text, ::this.filtering)}</div>,

            address != undefined &&
            <div key="addressFilter" className='filter-container'>{addressFilter(address, ::this.filtering)}</div>,

            unit != undefined &&
            <div key="unitFilter" className='filter-container'>{unitFilter(unit, ::this.filtering)}</div>,

            sales_agents != undefined &&
            <div key="sales_agentsFilter" className='filter-container'>{sales_agentsFilter(sales_agents, this.props.constants.agents, ::this.filterMultipleSelection)}</div>,

            listing_agents != undefined &&
            <div key="listing_agentsFilter" className='filter-container'>{listing_agentsFilter(listing_agents, this.props.constants.agents, ::this.filterMultipleSelection)}</div>,

            hoods != undefined &&
            <div key="hoodsFilter" className='filter-container'>{hoodsFilter(hoods, this.props.constants.neighborhoods, ::this.filterMultipleSelection)}</div>,

            price != undefined &&
            <div key="priceFilter" className='filter-container'>{priceFilter(price, ::this.filterRange)}</div>,

            price_per_bed != undefined &&
            <div key="price_per_bedFilter" className='filter-container'>{price_per_bedFilter(price_per_bed, ::this.filterRange)}</div>,

            beds != undefined &&
            <div key="bedsFilter" className='filter-container'>{bedsFilter(beds, ::this.filterMultipleSelection)}</div>,

            baths != undefined &&
            <div key="bathsFilter" className='filter-container'>{bathsFilter(baths, ::this.filterMultipleSelection)}</div>,

            listing_type != undefined &&
            <div key="listing_typeFilter" className='filter-container'>{listing_typeFilter(listing_type, this.props.constants.listing_types, ::this.filterOneSelection)}</div>,

            listing_id != undefined &&
            <div key="listing_idFilter" className='filter-container'>{listing_idFilter(listing_id, ::this.filtering)}</div>,

            lease_id != undefined &&
            <div key="lease_idFilter" className='filter-container'>{lease_idFilter(lease_id, ::this.filtering)}</div>,

            lease_status != undefined &&
            <div key="lease_statusFilter" className='filter-container'>{lease_statusFilter(lease_status, this.props.constants.lease_status, ::this.filterMultipleSelection)}</div>,

            size != undefined &&
            <div key="sizeFilter" className='filter-container'>{sizeFilter(size, ::this.filterRange)}</div>,

            pets_allowed != undefined &&
            <div key="pets_allowedFilter" className='filter-container'>{pets_allowedFilter(pets_allowed, this.props.constants.pets_allowed, ::this.filterOneSelection)}</div>,

            amenities != undefined &&
            <div key="amenitiesFilter" className='filter-container'>{amenitiesFilter(amenities, this.props.constants.amenities, ::this.filterMultipleSelection)}</div>,

            nofeeonly != undefined &&
            <div key="nofeeonlyFilter" className='filter-container'>{nofeeonlyFilter(nofeeonly, ::this.filterToggle)}</div>,

            owner_pays != undefined &&
            <div key="owner_paysFilter" className='filter-container'>{owner_paysFilter(owner_pays, ::this.filterToggle)}</div>,

            exclusive != undefined &&
            <div key="exclusiveFilter" className='filter-container'>{exclusiveFilter(exclusive, ::this.filterToggle)}</div>,

            vacant != undefined &&
            <div key="vacantFilter" className='filter-container'>{vacantFilter(vacant, ::this.filterToggle)}</div>,

            draft_listings != undefined &&
            <div key="draft_listingsFilter" className='filter-container'>{draft_listingsFilter(draft_listings, ::this.filterToggle)}</div>,

            only_active != undefined &&
            <div key="only_activeFilter" className='filter-container'>{only_activeFilter(only_active, ::this.filterToggle)}</div>,

            user_type != undefined &&
            <div key="user_typeFilter" className='filter-container'>{user_typeFilter(user_type, this.props.constants.user_type, ::this.filterOneSelection)}</div>,

            date_available != undefined &&
            <div key="date_availableFilter" className='filter-container'>{date_availableFilter(date_available, ::this.changeDate)}</div>,
        ]
    }
