...
window.JS_LOAD_FAILURE_MSG = 'The UI failed to load due to an error.' + (window.DEBUG ? ' Try rebuilding JS files with `<b>webpack</b>`.' : '')
...
window.onerror = function(error, url, line, python_exc) {
    ...
    if (show_advanced) {
        $('#react-loading .failed').append('<br><br><b>' + url + ':'+ (line || 'stderr') + '</b><br><br><pre>' + JSON.stringify(error).replace(/ \/.+?grater-django\//gim, " ") + '</pre><small>' + ERROR_HELP_MSG + '</small>');
    }
